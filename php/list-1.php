...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["num"])) {
    ...
    $sql_update = "UPDATE `cart` SET `state`='$state' WHERE `num`='$num'";
    $result = $seldb->query($sql_update);

    if ($result) {
        echo "<script>alert('已成功修改');location.href='list.php';</script>";
    } else {
        echo "<script>alert('修改失敗：" . $seldb->error . "');location.href='list.php';</script>";
    }
